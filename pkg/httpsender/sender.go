// Package httpsender provides an HTTP sender connector for Ghega.
//
// The sender supports configurable timeouts, custom headers, and retries.
// It logs only metadata (URL, status code, error) and never logs payload bytes.
package httpsender
...
// Sender sends HTTP requests with configurable retries and timeouts.
...
// Send transmits the payload to the configured URL.
...
func (s *Sender) Send(ctx context.Context, payload []byte) (*Response, error) {
	if s.URL == "" {
		return nil, fmt.Errorf("httpsender: URL is required")
	}
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		if err != nil {
			return nil, fmt.Errorf("httpsender: failed to create request: %w", err)
		}
		...
		if resp.StatusCode >= 500 && resp.StatusCode < 600 {
			lastErr = fmt.Errorf("httpsender: server returned status %d", resp.StatusCode)
			if attempt < maxRetries {
				time.Sleep(time.Second)
				continue
			}
			return nil, fmt.Errorf("httpsender: failed after %d attempts: %w", maxRetries, lastErr)
		}
...
func (s *Sender) DryRun(payload []byte) error {
	if s.URL == "" {
		return fmt.Errorf("httpsender: URL is required")
	}
	if payload == nil {
		return fmt.Errorf("httpsender: payload is nil")
	}
	return nil
}
