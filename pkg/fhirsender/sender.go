...
//
// The sender transmits FHIR JSON (individual resources or Bundles) to an
// external FHIR server with the correct Content-Type and Accept headers.
// It supports configurable timeouts, custom headers, and retries.
...
// Sender sends FHIR payloads to a target server.
...
func (s *Sender) Send(ctx context.Context, payload []byte) (*Response, error) {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		if err != nil {
			lastErr = err
			logger.Error("failed to read FHIR response body",
				slog.String("url", s.URL),
		...
		// Return error for 4xx status codes (no retry)
