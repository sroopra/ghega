...
//
// The sender transmits FHIR payloads to a configured URL with the correct
// FHIR content-type headers. It supports configurable timeouts, custom headers,
// and retries. It logs only metadata (URL, method, status code, error) and
...
// Sender sends FHIR requests with configurable retries and timeouts.
// It sets the appropriate FHIR content-type headers and never logs payload bytes.
type Sender struct {
	URL     string
	Method  string
...
// It sets Content-Type: application/fhir+json; fhirVersion=4.0 and
// Accept: application/fhir+json. It retries on transport errors or 5xx status
// codes up to the configured retry count.
...
func (s *Sender) Send(ctx context.Context, payload []byte) (*Response, error) {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		// Return error on 4xx status codes (no retry)
