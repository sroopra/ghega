...
//
// The sender transmits FHIR JSON resources and Bundles to external FHIR servers
// with the correct Content-Type and Accept headers. It supports configurable
// timeouts, custom headers, and retries. It logs only metadata (URL, status
// code, error, attempt count) and never logs payload bytes.
package fhirsender
...
// Sender sends FHIR JSON payloads to a FHIR server with correct headers
// and retry logic. It never logs payload bytes.
type Sender struct {
	URL     string
	Method  string
...
// Send transmits the FHIR JSON payload to the configured URL.
...
func (s *Sender) Send(ctx context.Context, payload []byte) (*Response, error) {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		// Apply custom headers first so FHIR headers take precedence
