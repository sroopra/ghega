...
// syntheticPayload simulates FHIR payload bytes that must never appear in log output.
const syntheticPayload = `{"resourceType":"Patient","id":"synthetic-patient-001","name":[{"family":"Doe","given":["Jane"]}],"identifier":[{"system":"http://hospital.example.org/mrn","value":"87654321"}]}`
...
func TestSend_NoRetryOnClientError(t *testing.T) {
	...
	_, err := sender.Send(context.Background(), []byte(`{}`))
	if err != nil {
		t.Fatalf("unexpected error on 4xx: %v", err)
	}
...
func TestDryRun_ValidPayload(t *testing.T) {
	sender := &Sender{URL: "http://fhir.example.com/R4"}
