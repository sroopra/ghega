...
const syntheticPayload = `{"resourceType":"Patient","id":"p-001","name":[{"family":"Testpatient"}]}`

func TestSend_SuccessWithPatientJSON(t *testing.T) {
...
func TestSend_UsesBundleToJSON(t *testing.T) {
	var receivedBody []byte
	...
	b := &fhir.Bundle{
		...
		Entry: []fhir.BundleEntry{
			{
				Resource: json.RawMessage(`{"resourceType":"Patient","id":"p-001"}`),
			},
		},
	}
	payload, err := fhir.BundleToJSON(b)
	if err != nil {
	...
	if parsed["resourceType"] != "Bundle" {
		t.Errorf("expected resourceType Bundle, got %v", parsed["resourceType"])
	}
}
...
func TestSend_NoRetryOnClientError(t *testing.T) {
	...
	resp, err := sender.Send(context.Background(), []byte(`{}`))
	if err == nil {
	...
	if resp != nil {
		t.Errorf("expected nil response on 4xx error, got %+v", resp)
	}
...
func TestSender_CustomValues(t *testing.T) {
	...
	if sender.method() != http.MethodPut {
		t.Errorf("expected method PUT, got %q", sender.method())
	}
