...
const syntheticPayload = `{"resourceType":"Patient","id":"synthetic-001","name":[{"family":"Doe","given":["Jane"]}]}`
...
func TestSend_SuccessWithCorrectHeaders(t *testing.T) {
...
func TestSend_BundlePayload(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"resourceType":"Bundle","id":"resp-1"}`))
	}))
	defer server.Close()

	bundle := []byte(`{"resourceType":"Bundle","type":"transaction","entry":[{"resource":{"resourceType":"Patient","id":"p1"}}]}`)
